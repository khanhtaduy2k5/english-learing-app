...
export default function VocabularyPage() {
  ...
  const getPosColor = (pos: string) =>
    ...
    "text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/10";

  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-8 text-slate-800 dark:text-slate-100 min-h-screen animate-fadeIn">
      {/* Header Banner */}
      ...
      <div className="flex gap-3">
        <div className="relative flex-1 group">
          ...
          <input
            ...
            className="w-full pl-12 pr-10 py-3.5 bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 group-hover:border-slate-300 dark:group-hover:border-white/10 focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/30 rounded-2xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none transition-all text-sm shadow-xl shadow-indigo-500/5 focus:bg-white dark:focus:bg-transparent"
          />
          {query && (
            <button
      ...
      {/* Loading Skeleton */}
      {isLoading && (
        <div className="p-8 rounded-3xl bg-slate-50 dark:bg-white/[0.01] border border-slate-200 dark:border-white/5 space-y-6 animate-pulse">
          <div className="flex justify-between items-center">
            <div className="space-y-2">
              <div className="h-8 w-48 bg-slate-200 dark:bg-white/5 rounded-lg" />
              <div className="h-4 w-32 bg-slate-200 dark:bg-white/5 rounded-lg" />
            </div>
            <div className="h-10 w-10 bg-slate-200 dark:bg-white/5 rounded-full" />
          </div>
          <div className="h-px bg-slate-200 dark:bg-white/5" />
          <div className="space-y-4">
            <div className="h-5 w-24 bg-slate-200 dark:bg-white/5 rounded-md" />
            <div className="h-16 bg-slate-200 dark:bg-white/5 rounded-2xl" />
          </div>
        </div>
      )}
      ...
      {/* Result View */}
      {entry && !isLoading ? (
        <div className="p-6 md:p-8 rounded-3xl bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 shadow-xl shadow-indigo-500/5 dark:shadow-2xl space-y-6 animate-fadeIn">
          {/* Word Header */}
          <div className="flex items-center justify-between flex-wrap gap-4 border-b border-slate-100 dark:border-white/5 pb-6">
            <div>
              <h2 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight capitalize leading-none">
                {entry.word}
              </h2>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${
                    activeTab === i
                      ...
                      : "bg-slate-50 dark:bg-white/5 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/5 hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white"
                  }`}
                >
            ...
            <div className="space-y-6">
              ...
              <ol className="space-y-4">
                {entry.meanings[activeTab].definitions.slice(0, 5).map((def, idx) => (
                  <li key={idx} className="flex gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-white/[0.01] border border-slate-200 dark:border-white/5 hover:border-indigo-500/15 transition-all">
                    ...
                    <div className="space-y-2">
                      <p className="text-slate-800 dark:text-white text-sm leading-relaxed">{def.definition}</p>
                      {def.example && (
                        <div className="text-slate-500 dark:text-slate-400 text-xs italic border-l-2 border-indigo-500/30 pl-3 leading-relaxed">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {entry.meanings[activeTab].synonyms.slice(0, 8).map((syn) => (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl text-xs bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 text-slate-600 dark:text-slate-300 hover:bg-indigo-500/10 hover:border-indigo-500/20 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all font-semibold"
                      >
                        {syn}
                      </button>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {entry.meanings[activeTab].antonyms.slice(0, 8).map((ant) => (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl text-xs bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 text-slate-600 dark:text-slate-300 hover:bg-rose-500/10 hover:border-rose-500/20 hover:text-rose-600 dark:hover:text-rose-400 transition-all font-semibold"
                      >
                        {ant}
                      </button>
                    ))}
          ...
          {/* Sources URL */}
          {entry.sourceUrls?.[0] && (
            <div className="text-[10px] text-slate-500 flex items-center gap-1 pt-4 border-t border-slate-100 dark:border-white/5">
              <span>Nguồn tài liệu học thuật:</span>
              <a
                href={entry.sourceUrls[0]}
                target="_blank"
                rel="noopener noreferrer"
                className="text-slate-500 hover:text-indigo-500 underline transition-colors"
              >
                {entry.sourceUrls[0]}
              </a>
            </div>
        ...
        <div className="p-8 rounded-3xl bg-white dark:bg-white/[0.01] border border-slate-200 dark:border-white/5 text-center py-16 space-y-6 shadow-xl shadow-indigo-500/5 dark:shadow-none">
          ...
          <div>
            <h3 className="text-slate-900 dark:text-white font-bold text-lg">Bắt đầu tra cứu từ vựng của bạn</h3>
            <p className="text-slate-600 dark:text-slate-500 text-sm max-w-sm mx-auto mt-1 leading-relaxed">

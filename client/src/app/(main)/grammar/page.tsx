...
export default function GrammarPage() {
  ...
  const allLevels = [
    "All",
    ...Array.from(new Set(rules.map((r) => r.level).filter(Boolean))),
  ];
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Dynamic English structures direct from database
            </p>
          </div>
        </div>
      </div>
        ...
        {allLevels.map((lvl) => (
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 ${
              selectedLevel === lvl
                ? "bg-gradient-to-r from-amber-500/30 to-orange-500/20 text-foreground dark:text-white border border-amber-500/30 shadow-lg shadow-amber-500/5"
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 space-y-4">
          ...
          <p className="text-muted-foreground text-sm font-medium">
            Loading grammar database...
          </p>
        </div>
      ) : filteredRules.length === 0 ? (
        <div className="text-center py-16 bg-card/70 dark:bg-white/[0.02] border border-border rounded-2xl p-8">
          <svg
          ...
          <p className="text-muted-foreground text-sm">
            Please change the filter level or check back later.
          </p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 gap-4">
            {paginatedRules.map((rule) => {
              const dc = difficultyConfig[rule.level] || {
              ...
              return (
                <div
                  key={rule.id}
                  className={`rounded-2xl bg-card/70 dark:bg-white/[0.03] border transition-all duration-300 overflow-hidden ${
                    isExpanded
                      ? "border-indigo-500/30 bg-card dark:bg-white/[0.05] shadow-xl shadow-indigo-950/10"
                  ...
                  {isExpanded && (
                    <div className="px-6 pb-6 pt-0 border-t border-border">
                      <div className="pt-6 space-y-6">
                        {/* Rule block */}
                        <div className="p-5 rounded-2xl bg-slate-100 dark:bg-slate-950/40 border border-border">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {rule.examples.map((ex: any, idx: number) => {
                                ...
                                return (
                                  <div
                                    key={idx}
                                    className="p-4 rounded-xl bg-card/80 dark:bg-white/[0.02] border border-border"
                                  >
                          ...
                          <div className="pt-4 border-t border-border">
                            {activeQuizId !== rule.id ? (
                              ...
                              >
                                ...
                                Start Practice Quiz ({rule.questions.length}{" "}
                                Questions)
                              </button>
                            ) : (
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    {showFeedback && (
                                      <div className="p-4 rounded-xl bg-white/5 border border-white/5 animate-slide-up space-y-2">
                                        <p
                                          className={`text-xs font-bold ${selectedAnswer === currentQuestion.answer ? "text-emerald-400" : "text-rose-400"}`}
                                        >
                                          {selectedAnswer ===
                                          currentQuestion.answer
                                            ? "✓ Correct!"
                                        ...
                                        <button
                                          onClick={() =>
                                            nextQuestion(activeQuestions.length)
                                          }
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-1 mt-6">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-center">
                <button
                  ...
                  className="px-3 py-2 rounded-lg text-xs font-semibold border border-border bg-card/70 text-foreground disabled:opacity-40 disabled:cursor-not-allowed hover:bg-card transition-colors"
                >
                  Trước
                </button>

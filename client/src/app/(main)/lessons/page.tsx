...
export default function LessonsPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 bg-gray-950 text-slate-100 min-h-screen">
      {/* Header Banner */}
      <div className="relative p-6 md:p-8 rounded-3xl overflow-hidden bg-gradient-to-r from-indigo-950/40 via-purple-950/20 to-gray-950 border border-white/5 shadow-2xl backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h1 className="text-2xl md:text-4xl font-extrabold tracking-tight text-white">Khóa học tiếng Anh</h1>
            <p className="text-slate-400 text-xs md:text-sm mt-0.5">Khám phá và bắt đầu học các chủ đề nâng cao phản hồi động.</p>
          </div>
        </div>
      </div>

      {/* Filter Row */}
      <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 shadow-xl flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/5 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500/40 focus:ring-1 focus:ring-indigo-500/20 text-sm transition-all"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="w-full bg-white/5 border border-white/5 rounded-xl text-xs text-slate-300 px-3 py-2.5 focus:outline-none focus:border-indigo-500/40"
            >
              {skills.map((sk) => (
                <option key={sk.id} value={sk.id} className="bg-gray-900 text-slate-300">
                  {sk.label}
                </option>
              ))}
            </select>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((n) => (
            <div key={n} className="bg-white/[0.01] border border-white/5 rounded-3xl p-6 space-y-4 animate-pulse">
              <div className="flex justify-between items-start">
                <div className="h-6 w-28 bg-white/5 rounded-lg"></div>
                <div className="h-5 w-14 bg-white/5 rounded-lg"></div>
              </div>
              <div className="space-y-2">
                <div className="h-6 w-full bg-white/5 rounded-lg"></div>
                <div className="h-4 w-5/6 bg-white/5 rounded-lg"></div>
              </div>
              <div className="h-10 w-full bg-white/5 rounded-xl pt-4"></div>
            </div>
          ))}
        </div>
      ) : filteredLessons.length === 0 ? (
        <div className="p-16 rounded-3xl bg-white/[0.01] border border-white/5 text-center flex flex-col items-center justify-center">
          <span className="text-6xl mb-4">🔍</span>
          <h3 className="text-white font-bold text-lg mb-2">Không Tìm Thấy Bài Học Nào</h3>
          ...
          {filteredLessons.map((lesson) => {
            ...
            return (
              <Link href={`/lessons/${lesson.id}`} key={lesson.id} className="block group">
                <div className="h-full bg-white/[0.02] hover:bg-white/[0.03] border border-white/5 hover:border-indigo-500/20 rounded-3xl p-6 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl shadow-md">
                  <div>
                    {/* Level & Skill badges */}
                    <div className="flex justify-between items-center mb-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-bold text-slate-300">{skillConfig.label}</span>
                      </div>
                    ...
                    {/* Title */}
                    <h2 className="text-lg font-bold text-white group-hover:text-indigo-400 transition-colors leading-snug mb-2 line-clamp-1">
                      {lesson.title}
                    </h2>

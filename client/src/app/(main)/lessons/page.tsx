...
export default function LessonsPage() {
  ...
  const levels = [
    "All",
    ...Array.from(new Set(lessons.map((l) => l.level).filter(Boolean))),
  ];
  const skills = [
    { id: "All", label: "Tất cả" },
  ...
  const filteredLessons = lessons.filter((lesson) => {
    ...
    const matchSkill =
      selectedSkill === "All" ||
      lesson.skill.toLowerCase() === selectedSkill.toLowerCase();
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 text-slate-800 dark:text-slate-100 min-h-screen">
      ...
      <div className="relative p-6 md:p-8 rounded-3xl overflow-hidden bg-gradient-to-r from-indigo-500/5 via-purple-500/5 to-slate-500/5 dark:from-indigo-950/40 dark:via-purple-950/20 dark:to-gray-950 border border-slate-200/80 dark:border-white/5 shadow-xl shadow-indigo-500/5 dark:shadow-2xl backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3 mb-3">
          <span className="text-3xl bg-indigo-500/10 w-12 h-12 rounded-2xl flex items-center justify-center border border-indigo-500/10">
            📚
          </span>
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400 text-xs md:text-sm mt-0.5">
              Khám phá và bắt đầu học các chủ đề nâng cao phản hồi động.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl p-5 shadow-xl flex flex-col gap-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-3">
          <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
            <SlidersHorizontal className="w-4 h-4 text-indigo-400" /> Bộ lọc bài
            học
          </h3>
            ...
            >
              {skills.map((sk) => (
                <option
                  key={sk.id}
                  value={sk.id}
                  className="bg-white dark:bg-gray-900 text-slate-700 dark:text-slate-300"
                >
                  {sk.label}
                </option>
              ))}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((n) => (
            <div
              key={n}
              className="bg-slate-50 dark:bg-white/[0.01] border border-slate-200 dark:border-white/5 rounded-3xl p-6 space-y-4 animate-pulse"
            >
                    ...
                    <div>
                      ...
                      <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-1.5">
                          <span
                            className={`p-1.5 rounded-lg ${skillConfig.bg} ${skillConfig.color} border ${skillConfig.border}`}
                          >
                      ...
                      <p className="text-slate-600 dark:text-slate-400 text-xs md:text-sm leading-relaxed mb-6 line-clamp-2">
                        {lesson.description ||
                          "Không có mô tả chi tiết bài học."}
                      </p>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-1">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-center">
                <button
                  ...
                  className="px-3 py-2 rounded-lg text-xs font-semibold border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-700 dark:text-slate-300 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-50 dark:hover:bg-white/10 transition-colors"
                >
                  Trước
                </button>

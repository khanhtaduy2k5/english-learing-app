...
Stores user account information. Managed by JPA/Hibernate (`ddl-auto: update`).

```sql
CREATE TABLE app_users (
  id VARCHAR(36) PRIMARY KEY,      -- UUID generated by @PrePersist
...
- `id`: UUID string (36 chars), auto-generated before persist
- `name`: User's full name
- `email`: User email (unique, used for login lookup)
